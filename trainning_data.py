...
data_dir_mother = pathlib.Path('mini_speech_commands_Function')
...
data_dir = pathlib.Path('input_Function')
...
original_label = np.asarray([0]*len(input1) + [1]*len(input2)+ [2]*len(input3)+ [3]*len(input4)).T #+ [4]*len(input5)+[5]*len(input6)+ 
                             #[6]*len(input7)).T#+ [7]*len(input8)+ [8]*len(input9)+ [9]*len(input10)).T# [11]*len(input12)).T
x = np.concatenate((input1,input2,input3,input4), axis = 0).T #input6,input7,input8,input9,input10,input11,input12), axis = 0).T
...
while (E>E_stop) and (epod<epod_max): ## initialize the Stopping Condition
  ...
  for i in range(x.shape[1]):
    ...
    E = E + 0.5*((e[0])**2 + (e[1])**2 + (e[2])**2 +(e[3])**2)
...
plt.plot(lam_store)
plt.grid(True) 
savemat('coeffs_Function\weight.mat', mdict={'w': w})
savemat('coeffs_Function\Lambda.mat', mdict={'lambda': Lambda})
